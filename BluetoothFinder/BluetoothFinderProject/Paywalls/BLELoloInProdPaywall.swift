...
struct BLELoloInProdPaywall: View {
            ...
            ZStack {
                VStack(spacing: 8) {
                    ...
                    Button(action: {
                        ...
                        HStack(spacing: 8) {
                            Text(paywallButtonTitle ?? "Subscribe for \(weeklyPrice)/week \nAuto renewable. Cancel anytime")
...
//Private extention
private extension BLELoloInProdPaywall {
    
    private func updatePrices() {
        Task {
            let price = await apphudService.getProductPrice(type: .weekly) ?? "..."
    ...
    private func subButtonDidTap() {
        let type: ApphudService.PurchaseType = .weekly
        Task {
